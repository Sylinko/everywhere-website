---
title: 助手
description: 在 Everywhere 中创建、配置和管理您的 AI 助手。
icon: Brain
---

import { Table } from "@/components/common/table"

您可以通过 Everywhere 主界面侧边栏的 **助手** 页面来管理和配置您的助手。

## 设置项

<Callout type="info" title="测试连通性">
在配置助手时，点击右上角的 **测试连通性** 即可快速验证 API 信息与网络环境；请注意该操作会发送简单请求并可能产生少量费用，若失败请核对配置或尝试切换网络。
</Callout>
...
系统提示词用于指导助手的行为和回答风格。您可以根据需要自定义系统提示词，以便让助手更好地符合您的需求。

<Callout type="info" title="占位符">
我们为您提供了一些占位符，您可以在系统提示词中使用它们来动态插入相关信息：
- `{Time}`：当前完整时间 (例如: 2026年4月17日 14:23:45)
- `{Date}`：当前日期 (例如: 2026年4月17日)
- `{OS}`：当前操作系统名称
- `{SystemLanguage}`：系统语言 *与 Everywhere 界面语言一致*
- `{WorkingDirectory}`：当前工作目录路径 *通常在`plugins`文件夹下以当天日期命名的文件夹形式出现*
- `{SkillsPrompt}`：您已启用的技能生成的提示词，可用于为助手提供额外信息。

大部分情况下推荐试用`{Date}`而不是`{Time}`，以增加模型的缓存命中率。
</Callout>
...
您可以选择接入我们的服务以使用 **原生模式**。如果您有其他模型提供商的 API 密钥 (BYOK)，也可以选择 **预设模式** 或 **进阶模式** 来配置助手。
...
- **支持使用工具**  
  启用此选项后，模型使用你配置的工具。**如果模型不支持使用工具，开启此选项可能会导致请求失败。**
- **支持深度思考**  
...
通常情况下，数值越高，回答越具创造力和想象力，数值越低，则越趋严谨。部分模型数值超过 1.0 可能导致输出乱码。通常建议不要同时更改 top_p。如果遇到错误，请尝试还原默认值。

<Callout type="info" title="提示">
- 点击右侧重置按钮后，则不会在请求中包含该参数。
- 如果没有看到此设置，则说明所选模型不支持调整该参数。
</Callout>

#### Top-P ("思维开放度")

用于控制模型输出文本的多样性。通常情况下，数值越高，模型会接受更多潜在回答，数值越低，则更倾向于最可能的回答。通常建议不要同时更改 temperature。如果遇到错误，请尝试还原默认值。
...
对于支持动态开关思考的模型，可以使用此选项关闭模型思考。留空则不会在请求体中包含该参数。

<Callout type="info">
如果设置为其他值，则该参数在请求体中的表现因 API 协议而异，如果效果不及预期，请尝试再次调整。
</Callout>

#### 推理强度 (reasoning_effort)

选择自定义推理强度。留空则不会在请求体中包含该参数。
...
<Callout type="info">
如果设置，则该参数在请求体中的表现因 API 协议而异，如果效果不及预期，请尝试再次调整。
</Callout>
