---
title: Web Browser
description: Activate the web browser tool to grant your assistant real-time internet access and search.
icon: PanelsTopLeft
---

import { HorizontalImg } from '@/components/common/horizontal-img';
import { PermissionBadge } from '@/components/common/permissions';

## Functions

| Function | Description | Permission(s) |
| -------- | ----------- | ----------------- |
| Web Search | Perform a web search. | <PermissionBadge risk="low">Access Network</PermissionBadge> |
| Visit Webpage | Capture a snapshot of a web page's content and basic info. | <PermissionBadge risk="low">Access Network</PermissionBadge> |

## Configuration

You can choose to use Google, Brave, Bocha, Tavily, Jina, UniFuncs or SearXNG as your search engine.

### Via Google

<Callout type="warning" title="Important Notice">
Google has announced that the Custom Search JSON API will shut down on January 1, 2027. The information in this tutorial is currently outdated. We highly recommend using alternative search services provided above.

See [Google's announcement](https://programmablesearchengine.googleblog.com/2026/01/updates-to-our-web-search-products.html) for more details.
</Callout>

This tutorial will guide you through the process of using [Google](https://developers.google.com/custom-search/v1/overview) as a web search service in Everywhere.

<Callout type="tip">
Google's Custom Search JSON API offers 100 free search queries per day ([see developer documentation](https://developers.google.com/custom-search/v1/overview))
</Callout>

<Callout type="warning">
The Google search service currently only supports access in some countries and regions. If your region is not supported, it is recommended to use other search services.
</Callout>

#### Preparation [!toc]

- A Google account
- If you have already created a project in Google Cloud, you can use the existing project directly.

#### Steps [!toc]

- Go to the [Google Cloud Console](https://console.cloud.google.com/) and log in to your account.
- After logging in, find the current default project, usually "**My First Project**," in the top left corner of the page. Click it to open the **Project selector**.

<HorizontalImg
    src="/model-provider/google/project-manager.webp"
    alt="Project Manager"
    width="600px"
  />

- In the Project selector, click the "**New Project**" button in the top right corner. This will take you to a new page where you can enter a project name. The organization field can be left blank.

<HorizontalImg
    src="/model-provider/google/create-project.webp"
    alt="Create project"
    width="500px"
  />

- After successfully creating the project, go to the [Programmable Search Engine Control Panel](https://programmablesearchengine.google.com/controlpanel/all) and click the **Add** button in the top right corner.

<HorizontalImg
    src="/chat-tools/web-browser/google/create-new-search-engine.webp"
    alt="Create search engine"
    width="500px"
  />

- In the form, enter a memorable name in the **Name your search engine** input box, select **Search the entire web** for **What to search?**, and leave the other settings at their defaults. After completing the CAPTCHA, click **Create**.

<HorizontalImg
    src="/chat-tools/web-browser/google/new-search-engine-form.webp"
    alt="Configure search engine"
    width="500px"
  />

- After the creation is successful, click **Customize** to enter the management page.

<HorizontalImg
    src="/chat-tools/web-browser/google/create-new-search-engine-success.webp"
    alt="Creation successful"
    width="500px"
  />

- In the **Basic** card, find the **Search engine ID** and copy it to the **Search engine ID** configuration item in Everywhere.

<HorizontalImg
    src="/chat-tools/web-browser/google/get-search-engine-id.webp"
    alt="Search engine ID"
    width="500px"
  />

- After visiting the [Custom Search JSON API Guide](https://developers.google.com/custom-search/v1/overview), find the **API key** section and click **Get a Key**.

<HorizontalImg
    src="/chat-tools/web-browser/google/get-api-key.webp"
    alt="API key"
    width="500px"
  />

- In the pop-up page, select the previously created project and click **NEXT**.

<HorizontalImg
    src="/chat-tools/web-browser/google/get-api-key-enable.webp"
    alt="Select project"
    width="500px"
  />

- Click **CONFIRM AND CONTINUE** to confirm enabling the Custom Search API in your project.

<HorizontalImg
    src="/chat-tools/web-browser/google/get-api-key-confirm.webp"
    alt="Confirm enable"
    width="500px"
  />

- After successfully enabling, click **SHOW KEY** to see the API key. Copy this key to the **API key** configuration item in Everywhere to use Google's search service.

<Callout type="warning">
Please make sure to keep the API key safe, as it will only be displayed once (if possible, also save a copy of the search engine ID for future use). If you accidentally close the dialog, you can click **Get a Key** again to regenerate a new key following the process.
</Callout>

<Callout type="warning" title="Privacy">
Please note that the API key is sensitive information. Do not disclose it to anyone or share it in public.
</Callout>

### Via Brave

This tutorial will guide you through the process of using [Brave](https://brave.com/search/api/) as a web search service in Everywhere.

<Callout type="warning">
The Brave search service currently only supports access in some countries and regions. If your region is not supported, it is recommended to use other search services.
</Callout>

#### Preparation [!toc]

- Register and log in to a Brave account
- May require a valid payment method, such as **Google Pay** or **debit/credit card**

#### Steps [!toc]

- Visit the [Brave Search API Dashboard](https://api-dashboard.search.brave.com/app/dashboard)

<HorizontalImg
    src="/chat-tools/web-browser/brave/homepage.webp"
    alt="Homepage"
    width="600px"
  />

- In the left sidebar of the page, click **Subscriptions**, select the subscription plan you need, and click **Subscribe**. *(Here, the selection is a free plan)*

<HorizontalImg
    src="/chat-tools/web-browser/brave/subscriptions.webp"
    alt="Subscription plans"
    width="600px"
  />

- Read and agree to the terms, enter the payment interface to select your payment method, and complete the subscription. Return to the **Subscriptions** page to ensure that your plan has been successfully subscribed.

<HorizontalImg
    src="/chat-tools/web-browser/brave/subscribed.webp"
    alt="Subscribed successfully"
    width="300px"
  />

- Click **API Keys** in the left sidebar, and then click **Add API key** in the upper right corner. In the pop-up dialog, fill in a memorable name in the **Name** field, select the plan you just subscribed to in the **Subscription** field, and click **Add**.

<HorizontalImg
    src="/chat-tools/web-browser/brave/create-api-key.webp"
    alt="Add API key"
    width="400px"
  />

- After successful creation, you will see the API key you just created. Click the **Copy** button to copy the key to the **API Key** configuration item in Everywhere to use Brave's search service.

<HorizontalImg
    src="/chat-tools/web-browser/brave/api-key.webp"
    alt="Copy API key"
    width="600px"
  />

<Callout type="warning" title="Privacy">
Please note that the API key is sensitive information. Do not disclose it to anyone or share it in public.
</Callout>

### Via Bocha

This tutorial will guide you step by step on how to use [Bocha](https://open.bochaai.com/) as a web search service in Everywhere.

<Callout type="warning">
Bocha service login only supports WeChat accounts. If WeChat login is not available in your region, it is recommended to use other search services.
</Callout>

#### Preparation [!toc]

- [Register and log in to a Bocha account](https://open.bochaai.com/login)

#### Steps [!toc]

- Visit the [Bocha AI Open Platform](https://api-dashboard.search.bochaai.com/app/dashboard) and click **API KEY管理** in the left sidebar.

<HorizontalImg
    src="/chat-tools/web-browser/bocha/homepage.webp"
    alt="Homepage"
  />

- Click **创建 API KEY**, and in the pop-up dialog, click **确认创建**.

<HorizontalImg
    src="/chat-tools/web-browser/bocha/create-api-key.webp"
    alt="Create API key"
    width="400px"
  />

- After successful creation, you will see the API key you just created. Click the copy button on the right to copy the key to the **API Key** configuration item in Everywhere to use Bocha's search service.

<HorizontalImg
    src="/chat-tools/web-browser/bocha/api-key.webp"
    alt="Copy API key"
    width="400px"
  />

<Callout type="error">
Please note that the API key is sensitive information, please do not disclose it to anyone or share it in public.
</Callout>

### Via Tavily

This tutorial will guide you step by step on how to use [Tavily](https://tavily.com/) as a web search service in Everywhere.

#### Preparation [!toc]

- Register and log in to Tavily in the upper right corner of the official website.

#### Steps [!toc]

- Visit the [Homepage](https://app.tavily.com/home) and find the **API Keys** section on the **Overview** page.

<HorizontalImg
    src="/chat-tools/web-browser/tavily/homepage.webp"
    alt="Homepage"
  />

- Click the plus button next to it to open the creation dialog. Enter a memorable name in the **Key Name** field and click **Create**.

<HorizontalImg
    src="/chat-tools/web-browser/tavily/create-api-key.webp"
    alt="Create API key"
    width="350px"
  />

- After successful creation, you will see the API key you just created. Click the copy button on the right to copy the key to the **API Key** configuration item in Everywhere to use Tavily's search service.

<HorizontalImg
    src="/chat-tools/web-browser/tavily/api-key.webp"
    alt="API key"
  />

<Callout type="warning" title="Privacy">
Please note that the API key is sensitive information. Do not disclose it to anyone or share it in public.
</Callout>

### Via Jina

This tutorial will guide you step by step on how to use [Jina](https://jina.ai/) as a web search service in Everywhere.

#### Preparation [!toc]

- Register and log in to Jina in the upper right corner of the official website.

#### Steps [!toc]

- Visit the [Dashboard](https://jina.ai/api-dashboard) and find the **Manage API Key** section in the sidebar.

<HorizontalImg
    src="/chat-tools/web-browser/jina/manage-api-key.webp"
    alt="Manage API Key"
    width="550px"
  />

- Jina ensures that your account has at least one API Key. Click the copy button next to the key to copy it to the **API Key** configuration item in Everywhere to use Jina's search service.

<HorizontalImg
    src="/chat-tools/web-browser/jina/get-api-key.webp"
    alt="Get API Key"
    width="500px"
  />

<Callout type="warning" title="Privacy">
Please note that the API key is sensitive information. Do not disclose it to anyone or share it in public.
</Callout>

### Via UniFuncs

This tutorial will guide you step by step on how to use [UniFuncs](https://www.unifuncs.com) as a web search service in Everywhere.

#### Preparation [!toc]

- Register and log in to UniFuncs in the upper right corner of the official website.

#### Steps [!toc]

- Visit [我的账户](https://www.unifuncs.com/account), and click **新建密钥** in the top right corner of the API Key section.

<HorizontalImg
    src="/chat-tools/web-browser/unifuncs/account.webp"
    alt="My Account"
  />

- In the pop-up dialog, enter a memorable name in the **密钥名称** field and click **创建**. Copy the **密钥内容** that appears to the **API 密钥** configuration item in Everywhere to use UniFuncs' search service.

<HorizontalImg
    src="/chat-tools/web-browser/unifuncs/get-api-key.webp"
    alt="Get API Key"
    width="400px"
  />

<Callout type="warning" title="Privacy">
Please note that the API key is sensitive information. Do not disclose it to anyone or share it in public.
</Callout>